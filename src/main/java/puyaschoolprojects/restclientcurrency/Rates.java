package puyaschoolprojects.restclientcurrency;

public class Rates {

    private Double FJD;

    private Double MXN;

    private Double STD;

    private Double LVL;

    private Double SCR;

    private Double CDF;

    private Double BBD;

    private Double GTQ;

    private Double CLP;

    private Double HNL;

    private Double UGX;

    private Double ZAR;

    private Double TND;

    private Double CUC;

    private Double BSD;

    private Double SLL;

    private Double SDG;

    private Double IQD;

    private Double CUP;

    private Double GMD;

    private Double TWD;

    private Double RSD;

    private Double DOP;

    private Double KMF;

    private Double MYR;

    private Double FKP;

    private Double XOF;

    private Double GEL;

    private Double BTC;

    private Double UYU;

    private Double MAD;

    private Double CVE;

    private Double TOP;

    private Double AZN;

    private Double OMR;

    private Double PGK;

    private Double KES;

    private Double SEK;

    private Double BTN;

    private Double UAH;

    private Double GNF;

    private Double ERN;

    private Double MZN;

    private Double SVC;

    private Double ARS;

    private Double QAR;

    private Double IRR;

    private Double MRO;

    private Double CNY;

    private Double THB;

    private Double UZS;

    private Double XPF;

    private Double BDT;

    private Double LYD;

    private Double BMD;

    private Double KWD;

    private Double PHP;

    private Double RUB;

    private Double PYG;

    private Double ISK;

    private Double JMD;

    private Double COP;

    private Double MKD;

    private Double USD;

    private Double DZD;

    private Double PAB;

    private Double GGP;

    private Double SGD;

    private Double ETB;

    private Double JEP;

    private Double KGS;

    private Double SOS;

    private Double VEF;

    private Double VUV;

    private Double LAK;

    private Double BND;

    private Double ZMK;

    private Double XAF;

    private Double LRD;

    private Double XAG;

    private Double CHF;

    private Double HRK;

    private Double ALL;

    private Double DJF;

    private Double ZMW;

    private Double TZS;

    private Double VND;

    private Double XAU;

    private Double AUD;

    private Double ILS;

    private Double GHS;

    private Double GYD;

    private Double KPW;

    private Double BOB;

    private Double KHR;

    private Double MDL;

    private Double IDR;

    private Double KYD;

    private Double AMD;

    private Double BWP;

    private Double SHP;

    private Double TRY;

    private Double LBP;

    private Double TJS;

    private Double JOD;

    private Double AED;

    private Double HKD;

    private Double RWF;

    private Double EUR;

    private Double LSL;

    private Double DKK;

    private Double CAD;

    private Double BGN;

    private Double MMK;

    private Double MUR;

    private Double NOK;

    private Double SYP;

    private Double IMP;

    private Double ZWL;

    private Double GIP;

    private Double RON;

    private Double LKR;

    private Double NGN;

    private Double CRC;

    private Double CZK;

    private Double PKR;

    private Double XCD;

    private Double ANG;

    private Double HTG;

    private Double BHD;

    private Double KZT;

    private Double SRD;

    private Double SZL;

    private Double LTL;

    private Double SAR;

    private Double TTD;

    private Double YER;

    private Double MVR;

    private Double AFN;

    private Double INR;

    private Double AWG;

    private Double KRW;

    private Double NPR;

    private Double JPY;

    private Double MNT;

    private Double AOA;

    private Double PLN;

    private Double GBP;

    private Double SBD;

    private Double BYN;

    private Double HUF;

    private Double BYR;

    private Double BIF;

    private Double MWK;

    private Double MGA;

    private Double XDR;

    private Double BZD;

    private Double BAM;

    private Double EGP;

    private Double MOP;

    private Double NAD;

    private Double NIO;

    private Double PEN;

    private Double NZD;

    private Double WST;

    private Double TMT;

    private Double CLF;

    private Double BRL;

    public Double getFJD() {
        return FJD;
    }

    public void setFJD(Double FJD) {
        this.FJD = FJD;
    }

    public Double getMXN() {
        return MXN;
    }

    public void setMXN(Double MXN) {
        this.MXN = MXN;
    }

    public Double getSTD() {
        return STD;
    }

    public void setSTD(Double STD) {
        this.STD = STD;
    }

    public Double getLVL() {
        return LVL;
    }

    public void setLVL(Double LVL) {
        this.LVL = LVL;
    }

    public Double getSCR() {
        return SCR;
    }

    public void setSCR(Double SCR) {
        this.SCR = SCR;
    }

    public Double getCDF() {
        return CDF;
    }

    public void setCDF(Double CDF) {
        this.CDF = CDF;
    }

    public Double getBBD() {
        return BBD;
    }

    public void setBBD(Double BBD) {
        this.BBD = BBD;
    }

    public Double getGTQ() {
        return GTQ;
    }

    public void setGTQ(Double GTQ) {
        this.GTQ = GTQ;
    }

    public Double getCLP() {
        return CLP;
    }

    public void setCLP(Double CLP) {
        this.CLP = CLP;
    }

    public Double getHNL() {
        return HNL;
    }

    public void setHNL(Double HNL) {
        this.HNL = HNL;
    }

    public Double getUGX() {
        return UGX;
    }

    public void setUGX(Double UGX) {
        this.UGX = UGX;
    }

    public Double getZAR() {
        return ZAR;
    }

    public void setZAR(Double ZAR) {
        this.ZAR = ZAR;
    }

    public Double getTND() {
        return TND;
    }

    public void setTND(Double TND) {
        this.TND = TND;
    }

    public Double getCUC() {
        return CUC;
    }

    public void setCUC(Double CUC) {
        this.CUC = CUC;
    }

    public Double getBSD() {
        return BSD;
    }

    public void setBSD(Double BSD) {
        this.BSD = BSD;
    }

    public Double getSLL() {
        return SLL;
    }

    public void setSLL(Double SLL) {
        this.SLL = SLL;
    }

    public Double getSDG() {
        return SDG;
    }

    public void setSDG(Double SDG) {
        this.SDG = SDG;
    }

    public Double getIQD() {
        return IQD;
    }

    public void setIQD(Double IQD) {
        this.IQD = IQD;
    }

    public Double getCUP() {
        return CUP;
    }

    public void setCUP(Double CUP) {
        this.CUP = CUP;
    }

    public Double getGMD() {
        return GMD;
    }

    public void setGMD(Double GMD) {
        this.GMD = GMD;
    }

    public Double getTWD() {
        return TWD;
    }

    public void setTWD(Double TWD) {
        this.TWD = TWD;
    }

    public Double getRSD() {
        return RSD;
    }

    public void setRSD(Double RSD) {
        this.RSD = RSD;
    }

    public Double getDOP() {
        return DOP;
    }

    public void setDOP(Double DOP) {
        this.DOP = DOP;
    }

    public Double getKMF() {
        return KMF;
    }

    public void setKMF(Double KMF) {
        this.KMF = KMF;
    }

    public Double getMYR() {
        return MYR;
    }

    public void setMYR(Double MYR) {
        this.MYR = MYR;
    }

    public Double getFKP() {
        return FKP;
    }

    public void setFKP(Double FKP) {
        this.FKP = FKP;
    }

    public Double getXOF() {
        return XOF;
    }

    public void setXOF(Double XOF) {
        this.XOF = XOF;
    }

    public Double getGEL() {
        return GEL;
    }

    public void setGEL(Double GEL) {
        this.GEL = GEL;
    }

    public Double getBTC() {
        return BTC;
    }

    public void setBTC(Double BTC) {
        this.BTC = BTC;
    }

    public Double getUYU() {
        return UYU;
    }

    public void setUYU(Double UYU) {
        this.UYU = UYU;
    }

    public Double getMAD() {
        return MAD;
    }

    public void setMAD(Double MAD) {
        this.MAD = MAD;
    }

    public Double getCVE() {
        return CVE;
    }

    public void setCVE(Double CVE) {
        this.CVE = CVE;
    }

    public Double getTOP() {
        return TOP;
    }

    public void setTOP(Double TOP) {
        this.TOP = TOP;
    }

    public Double getAZN() {
        return AZN;
    }

    public void setAZN(Double AZN) {
        this.AZN = AZN;
    }

    public Double getOMR() {
        return OMR;
    }

    public void setOMR(Double OMR) {
        this.OMR = OMR;
    }

    public Double getPGK() {
        return PGK;
    }

    public void setPGK(Double PGK) {
        this.PGK = PGK;
    }

    public Double getKES() {
        return KES;
    }

    public void setKES(Double KES) {
        this.KES = KES;
    }

    public Double getSEK() {
        return SEK;
    }

    public void setSEK(Double SEK) {
        this.SEK = SEK;
    }

    public Double getBTN() {
        return BTN;
    }

    public void setBTN(Double BTN) {
        this.BTN = BTN;
    }

    public Double getUAH() {
        return UAH;
    }

    public void setUAH(Double UAH) {
        this.UAH = UAH;
    }

    public Double getGNF() {
        return GNF;
    }

    public void setGNF(Double GNF) {
        this.GNF = GNF;
    }

    public Double getERN() {
        return ERN;
    }

    public void setERN(Double ERN) {
        this.ERN = ERN;
    }

    public Double getMZN() {
        return MZN;
    }

    public void setMZN(Double MZN) {
        this.MZN = MZN;
    }

    public Double getSVC() {
        return SVC;
    }

    public void setSVC(Double SVC) {
        this.SVC = SVC;
    }

    public Double getARS() {
        return ARS;
    }

    public void setARS(Double ARS) {
        this.ARS = ARS;
    }

    public Double getQAR() {
        return QAR;
    }

    public void setQAR(Double QAR) {
        this.QAR = QAR;
    }

    public Double getIRR() {
        return IRR;
    }

    public void setIRR(Double IRR) {
        this.IRR = IRR;
    }

    public Double getMRO() {
        return MRO;
    }

    public void setMRO(Double MRO) {
        this.MRO = MRO;
    }

    public Double getCNY() {
        return CNY;
    }

    public void setCNY(Double CNY) {
        this.CNY = CNY;
    }

    public Double getTHB() {
        return THB;
    }

    public void setTHB(Double THB) {
        this.THB = THB;
    }

    public Double getUZS() {
        return UZS;
    }

    public void setUZS(Double UZS) {
        this.UZS = UZS;
    }

    public Double getXPF() {
        return XPF;
    }

    public void setXPF(Double XPF) {
        this.XPF = XPF;
    }

    public Double getBDT() {
        return BDT;
    }

    public void setBDT(Double BDT) {
        this.BDT = BDT;
    }

    public Double getLYD() {
        return LYD;
    }

    public void setLYD(Double LYD) {
        this.LYD = LYD;
    }

    public Double getBMD() {
        return BMD;
    }

    public void setBMD(Double BMD) {
        this.BMD = BMD;
    }

    public Double getKWD() {
        return KWD;
    }

    public void setKWD(Double KWD) {
        this.KWD = KWD;
    }

    public Double getPHP() {
        return PHP;
    }

    public void setPHP(Double PHP) {
        this.PHP = PHP;
    }

    public Double getRUB() {
        return RUB;
    }

    public void setRUB(Double RUB) {
        this.RUB = RUB;
    }

    public Double getPYG() {
        return PYG;
    }

    public void setPYG(Double PYG) {
        this.PYG = PYG;
    }

    public Double getISK() {
        return ISK;
    }

    public void setISK(Double ISK) {
        this.ISK = ISK;
    }

    public Double getJMD() {
        return JMD;
    }

    public void setJMD(Double JMD) {
        this.JMD = JMD;
    }

    public Double getCOP() {
        return COP;
    }

    public void setCOP(Double COP) {
        this.COP = COP;
    }

    public Double getMKD() {
        return MKD;
    }

    public void setMKD(Double MKD) {
        this.MKD = MKD;
    }

    public Double getUSD() {
        return USD;
    }

    public void setUSD(Double USD) {
        this.USD = USD;
    }

    public Double getDZD() {
        return DZD;
    }

    public void setDZD(Double DZD) {
        this.DZD = DZD;
    }

    public Double getPAB() {
        return PAB;
    }

    public void setPAB(Double PAB) {
        this.PAB = PAB;
    }

    public Double getGGP() {
        return GGP;
    }

    public void setGGP(Double GGP) {
        this.GGP = GGP;
    }

    public Double getSGD() {
        return SGD;
    }

    public void setSGD(Double SGD) {
        this.SGD = SGD;
    }

    public Double getETB() {
        return ETB;
    }

    public void setETB(Double ETB) {
        this.ETB = ETB;
    }

    public Double getJEP() {
        return JEP;
    }

    public void setJEP(Double JEP) {
        this.JEP = JEP;
    }

    public Double getKGS() {
        return KGS;
    }

    public void setKGS(Double KGS) {
        this.KGS = KGS;
    }

    public Double getSOS() {
        return SOS;
    }

    public void setSOS(Double SOS) {
        this.SOS = SOS;
    }

    public Double getVEF() {
        return VEF;
    }

    public void setVEF(Double VEF) {
        this.VEF = VEF;
    }

    public Double getVUV() {
        return VUV;
    }

    public void setVUV(Double VUV) {
        this.VUV = VUV;
    }

    public Double getLAK() {
        return LAK;
    }

    public void setLAK(Double LAK) {
        this.LAK = LAK;
    }

    public Double getBND() {
        return BND;
    }

    public void setBND(Double BND) {
        this.BND = BND;
    }

    public Double getZMK() {
        return ZMK;
    }

    public void setZMK(Double ZMK) {
        this.ZMK = ZMK;
    }

    public Double getXAF() {
        return XAF;
    }

    public void setXAF(Double XAF) {
        this.XAF = XAF;
    }

    public Double getLRD() {
        return LRD;
    }

    public void setLRD(Double LRD) {
        this.LRD = LRD;
    }

    public Double getXAG() {
        return XAG;
    }

    public void setXAG(Double XAG) {
        this.XAG = XAG;
    }

    public Double getCHF() {
        return CHF;
    }

    public void setCHF(Double CHF) {
        this.CHF = CHF;
    }

    public Double getHRK() {
        return HRK;
    }

    public void setHRK(Double HRK) {
        this.HRK = HRK;
    }

    public Double getALL() {
        return ALL;
    }

    public void setALL(Double ALL) {
        this.ALL = ALL;
    }

    public Double getDJF() {
        return DJF;
    }

    public void setDJF(Double DJF) {
        this.DJF = DJF;
    }

    public Double getZMW() {
        return ZMW;
    }

    public void setZMW(Double ZMW) {
        this.ZMW = ZMW;
    }

    public Double getTZS() {
        return TZS;
    }

    public void setTZS(Double TZS) {
        this.TZS = TZS;
    }

    public Double getVND() {
        return VND;
    }

    public void setVND(Double VND) {
        this.VND = VND;
    }

    public Double getXAU() {
        return XAU;
    }

    public void setXAU(Double XAU) {
        this.XAU = XAU;
    }

    public Double getAUD() {
        return AUD;
    }

    public void setAUD(Double AUD) {
        this.AUD = AUD;
    }

    public Double getILS() {
        return ILS;
    }

    public void setILS(Double ILS) {
        this.ILS = ILS;
    }

    public Double getGHS() {
        return GHS;
    }

    public void setGHS(Double GHS) {
        this.GHS = GHS;
    }

    public Double getGYD() {
        return GYD;
    }

    public void setGYD(Double GYD) {
        this.GYD = GYD;
    }

    public Double getKPW() {
        return KPW;
    }

    public void setKPW(Double KPW) {
        this.KPW = KPW;
    }

    public Double getBOB() {
        return BOB;
    }

    public void setBOB(Double BOB) {
        this.BOB = BOB;
    }

    public Double getKHR() {
        return KHR;
    }

    public void setKHR(Double KHR) {
        this.KHR = KHR;
    }

    public Double getMDL() {
        return MDL;
    }

    public void setMDL(Double MDL) {
        this.MDL = MDL;
    }

    public Double getIDR() {
        return IDR;
    }

    public void setIDR(Double IDR) {
        this.IDR = IDR;
    }

    public Double getKYD() {
        return KYD;
    }

    public void setKYD(Double KYD) {
        this.KYD = KYD;
    }

    public Double getAMD() {
        return AMD;
    }

    public void setAMD(Double AMD) {
        this.AMD = AMD;
    }

    public Double getBWP() {
        return BWP;
    }

    public void setBWP(Double BWP) {
        this.BWP = BWP;
    }

    public Double getSHP() {
        return SHP;
    }

    public void setSHP(Double SHP) {
        this.SHP = SHP;
    }

    public Double getTRY() {
        return TRY;
    }

    public void setTRY(Double TRY) {
        this.TRY = TRY;
    }

    public Double getLBP() {
        return LBP;
    }

    public void setLBP(Double LBP) {
        this.LBP = LBP;
    }

    public Double getTJS() {
        return TJS;
    }

    public void setTJS(Double TJS) {
        this.TJS = TJS;
    }

    public Double getJOD() {
        return JOD;
    }

    public void setJOD(Double JOD) {
        this.JOD = JOD;
    }

    public Double getAED() {
        return AED;
    }

    public void setAED(Double AED) {
        this.AED = AED;
    }

    public Double getHKD() {
        return HKD;
    }

    public void setHKD(Double HKD) {
        this.HKD = HKD;
    }

    public Double getRWF() {
        return RWF;
    }

    public void setRWF(Double RWF) {
        this.RWF = RWF;
    }

    public Double getEUR() {
        return EUR;
    }

    public void setEUR(Double EUR) {
        this.EUR = EUR;
    }

    public Double getLSL() {
        return LSL;
    }

    public void setLSL(Double LSL) {
        this.LSL = LSL;
    }

    public Double getDKK() {
        return DKK;
    }

    public void setDKK(Double DKK) {
        this.DKK = DKK;
    }

    public Double getCAD() {
        return CAD;
    }

    public void setCAD(Double CAD) {
        this.CAD = CAD;
    }

    public Double getBGN() {
        return BGN;
    }

    public void setBGN(Double BGN) {
        this.BGN = BGN;
    }

    public Double getMMK() {
        return MMK;
    }

    public void setMMK(Double MMK) {
        this.MMK = MMK;
    }

    public Double getMUR() {
        return MUR;
    }

    public void setMUR(Double MUR) {
        this.MUR = MUR;
    }

    public Double getNOK() {
        return NOK;
    }

    public void setNOK(Double NOK) {
        this.NOK = NOK;
    }

    public Double getSYP() {
        return SYP;
    }

    public void setSYP(Double SYP) {
        this.SYP = SYP;
    }

    public Double getIMP() {
        return IMP;
    }

    public void setIMP(Double IMP) {
        this.IMP = IMP;
    }

    public Double getZWL() {
        return ZWL;
    }

    public void setZWL(Double ZWL) {
        this.ZWL = ZWL;
    }

    public Double getGIP() {
        return GIP;
    }

    public void setGIP(Double GIP) {
        this.GIP = GIP;
    }

    public Double getRON() {
        return RON;
    }

    public void setRON(Double RON) {
        this.RON = RON;
    }

    public Double getLKR() {
        return LKR;
    }

    public void setLKR(Double LKR) {
        this.LKR = LKR;
    }

    public Double getNGN() {
        return NGN;
    }

    public void setNGN(Double NGN) {
        this.NGN = NGN;
    }

    public Double getCRC() {
        return CRC;
    }

    public void setCRC(Double CRC) {
        this.CRC = CRC;
    }

    public Double getCZK() {
        return CZK;
    }

    public void setCZK(Double CZK) {
        this.CZK = CZK;
    }

    public Double getPKR() {
        return PKR;
    }

    public void setPKR(Double PKR) {
        this.PKR = PKR;
    }

    public Double getXCD() {
        return XCD;
    }

    public void setXCD(Double XCD) {
        this.XCD = XCD;
    }

    public Double getANG() {
        return ANG;
    }

    public void setANG(Double ANG) {
        this.ANG = ANG;
    }

    public Double getHTG() {
        return HTG;
    }

    public void setHTG(Double HTG) {
        this.HTG = HTG;
    }

    public Double getBHD() {
        return BHD;
    }

    public void setBHD(Double BHD) {
        this.BHD = BHD;
    }

    public Double getKZT() {
        return KZT;
    }

    public void setKZT(Double KZT) {
        this.KZT = KZT;
    }

    public Double getSRD() {
        return SRD;
    }

    public void setSRD(Double SRD) {
        this.SRD = SRD;
    }

    public Double getSZL() {
        return SZL;
    }

    public void setSZL(Double SZL) {
        this.SZL = SZL;
    }

    public Double getLTL() {
        return LTL;
    }

    public void setLTL(Double LTL) {
        this.LTL = LTL;
    }

    public Double getSAR() {
        return SAR;
    }

    public void setSAR(Double SAR) {
        this.SAR = SAR;
    }

    public Double getTTD() {
        return TTD;
    }

    public void setTTD(Double TTD) {
        this.TTD = TTD;
    }

    public Double getYER() {
        return YER;
    }

    public void setYER(Double YER) {
        this.YER = YER;
    }

    public Double getMVR() {
        return MVR;
    }

    public void setMVR(Double MVR) {
        this.MVR = MVR;
    }

    public Double getAFN() {
        return AFN;
    }

    public void setAFN(Double AFN) {
        this.AFN = AFN;
    }

    public Double getINR() {
        return INR;
    }

    public void setINR(Double INR) {
        this.INR = INR;
    }

    public Double getAWG() {
        return AWG;
    }

    public void setAWG(Double AWG) {
        this.AWG = AWG;
    }

    public Double getKRW() {
        return KRW;
    }

    public void setKRW(Double KRW) {
        this.KRW = KRW;
    }

    public Double getNPR() {
        return NPR;
    }

    public void setNPR(Double NPR) {
        this.NPR = NPR;
    }

    public Double getJPY() {
        return JPY;
    }

    public void setJPY(Double JPY) {
        this.JPY = JPY;
    }

    public Double getMNT() {
        return MNT;
    }

    public void setMNT(Double MNT) {
        this.MNT = MNT;
    }

    public Double getAOA() {
        return AOA;
    }

    public void setAOA(Double AOA) {
        this.AOA = AOA;
    }

    public Double getPLN() {
        return PLN;
    }

    public void setPLN(Double PLN) {
        this.PLN = PLN;
    }

    public Double getGBP() {
        return GBP;
    }

    public void setGBP(Double GBP) {
        this.GBP = GBP;
    }

    public Double getSBD() {
        return SBD;
    }

    public void setSBD(Double SBD) {
        this.SBD = SBD;
    }

    public Double getBYN() {
        return BYN;
    }

    public void setBYN(Double BYN) {
        this.BYN = BYN;
    }

    public Double getHUF() {
        return HUF;
    }

    public void setHUF(Double HUF) {
        this.HUF = HUF;
    }

    public Double getBYR() {
        return BYR;
    }

    public void setBYR(Double BYR) {
        this.BYR = BYR;
    }

    public Double getBIF() {
        return BIF;
    }

    public void setBIF(Double BIF) {
        this.BIF = BIF;
    }

    public Double getMWK() {
        return MWK;
    }

    public void setMWK(Double MWK) {
        this.MWK = MWK;
    }

    public Double getMGA() {
        return MGA;
    }

    public void setMGA(Double MGA) {
        this.MGA = MGA;
    }

    public Double getXDR() {
        return XDR;
    }

    public void setXDR(Double XDR) {
        this.XDR = XDR;
    }

    public Double getBZD() {
        return BZD;
    }

    public void setBZD(Double BZD) {
        this.BZD = BZD;
    }

    public Double getBAM() {
        return BAM;
    }

    public void setBAM(Double BAM) {
        this.BAM = BAM;
    }

    public Double getEGP() {
        return EGP;
    }

    public void setEGP(Double EGP) {
        this.EGP = EGP;
    }

    public Double getMOP() {
        return MOP;
    }

    public void setMOP(Double MOP) {
        this.MOP = MOP;
    }

    public Double getNAD() {
        return NAD;
    }

    public void setNAD(Double NAD) {
        this.NAD = NAD;
    }

    public Double getNIO() {
        return NIO;
    }

    public void setNIO(Double NIO) {
        this.NIO = NIO;
    }

    public Double getPEN() {
        return PEN;
    }

    public void setPEN(Double PEN) {
        this.PEN = PEN;
    }

    public Double getNZD() {
        return NZD;
    }

    public void setNZD(Double NZD) {
        this.NZD = NZD;
    }

    public Double getWST() {
        return WST;
    }

    public void setWST(Double WST) {
        this.WST = WST;
    }

    public Double getTMT() {
        return TMT;
    }

    public void setTMT(Double TMT) {
        this.TMT = TMT;
    }

    public Double getCLF() {
        return CLF;
    }

    public void setCLF(Double CLF) {
        this.CLF = CLF;
    }

    public Double getBRL() {
        return BRL;
    }

    public void setBRL(Double BRL) {
        this.BRL = BRL;
    }

    @Override
    public String toString() {
        return "ClassPojo [FJD = " + FJD + ", MXN = " + MXN + ", STD = " + STD + ", LVL = " + LVL + ", SCR = " + SCR + ", CDF = " + CDF + ", BBD = " + BBD + ", GTQ = " + GTQ + ", CLP = " + CLP + ", HNL = " + HNL + ", UGX = " + UGX + ", ZAR = " + ZAR + ", TND = " + TND + ", CUC = " + CUC + ", BSD = " + BSD + ", SLL = " + SLL + ", SDG = " + SDG + ", IQD = " + IQD + ", CUP = " + CUP + ", GMD = " + GMD + ", TWD = " + TWD + ", RSD = " + RSD + ", DOP = " + DOP + ", KMF = " + KMF + ", MYR = " + MYR + ", FKP = " + FKP + ", XOF = " + XOF + ", GEL = " + GEL + ", BTC = " + BTC + ", UYU = " + UYU + ", MAD = " + MAD + ", CVE = " + CVE + ", TOP = " + TOP + ", AZN = " + AZN + ", OMR = " + OMR + ", PGK = " + PGK + ", KES = " + KES + ", SEK = " + SEK + ", BTN = " + BTN + ", UAH = " + UAH + ", GNF = " + GNF + ", ERN = " + ERN + ", MZN = " + MZN + ", SVC = " + SVC + ", ARS = " + ARS + ", QAR = " + QAR + ", IRR = " + IRR + ", MRO = " + MRO + ", CNY = " + CNY + ", THB = " + THB + ", UZS = " + UZS + ", XPF = " + XPF + ", BDT = " + BDT + ", LYD = " + LYD + ", BMD = " + BMD + ", KWD = " + KWD + ", PHP = " + PHP + ", RUB = " + RUB + ", PYG = " + PYG + ", ISK = " + ISK + ", JMD = " + JMD + ", COP = " + COP + ", MKD = " + MKD + ", USD = " + USD + ", DZD = " + DZD + ", PAB = " + PAB + ", GGP = " + GGP + ", SGD = " + SGD + ", ETB = " + ETB + ", JEP = " + JEP + ", KGS = " + KGS + ", SOS = " + SOS + ", VEF = " + VEF + ", VUV = " + VUV + ", LAK = " + LAK + ", BND = " + BND + ", ZMK = " + ZMK + ", XAF = " + XAF + ", LRD = " + LRD + ", XAG = " + XAG + ", CHF = " + CHF + ", HRK = " + HRK + ", ALL = " + ALL + ", DJF = " + DJF + ", ZMW = " + ZMW + ", TZS = " + TZS + ", VND = " + VND + ", XAU = " + XAU + ", AUD = " + AUD + ", ILS = " + ILS + ", GHS = " + GHS + ", GYD = " + GYD + ", KPW = " + KPW + ", BOB = " + BOB + ", KHR = " + KHR + ", MDL = " + MDL + ", IDR = " + IDR + ", KYD = " + KYD + ", AMD = " + AMD + ", BWP = " + BWP + ", SHP = " + SHP + ", TRY = " + TRY + ", LBP = " + LBP + ", TJS = " + TJS + ", JOD = " + JOD + ", AED = " + AED + ", HKD = " + HKD + ", RWF = " + RWF + ", EUR = " + EUR + ", LSL = " + LSL + ", DKK = " + DKK + ", CAD = " + CAD + ", BGN = " + BGN + ", MMK = " + MMK + ", MUR = " + MUR + ", NOK = " + NOK + ", SYP = " + SYP + ", IMP = " + IMP + ", ZWL = " + ZWL + ", GIP = " + GIP + ", RON = " + RON + ", LKR = " + LKR + ", NGN = " + NGN + ", CRC = " + CRC + ", CZK = " + CZK + ", PKR = " + PKR + ", XCD = " + XCD + ", ANG = " + ANG + ", HTG = " + HTG + ", BHD = " + BHD + ", KZT = " + KZT + ", SRD = " + SRD + ", SZL = " + SZL + ", LTL = " + LTL + ", SAR = " + SAR + ", TTD = " + TTD + ", YER = " + YER + ", MVR = " + MVR + ", AFN = " + AFN + ", INR = " + INR + ", AWG = " + AWG + ", KRW = " + KRW + ", NPR = " + NPR + ", JPY = " + JPY + ", MNT = " + MNT + ", AOA = " + AOA + ", PLN = " + PLN + ", GBP = " + GBP + ", SBD = " + SBD + ", BYN = " + BYN + ", HUF = " + HUF + ", BYR = " + BYR + ", BIF = " + BIF + ", MWK = " + MWK + ", MGA = " + MGA + ", XDR = " + XDR + ", BZD = " + BZD + ", BAM = " + BAM + ", EGP = " + EGP + ", MOP = " + MOP + ", NAD = " + NAD + ", NIO = " + NIO + ", PEN = " + PEN + ", NZD = " + NZD + ", WST = " + WST + ", TMT = " + TMT + ", CLF = " + CLF + ", BRL = " + BRL + "]";
    }
}
